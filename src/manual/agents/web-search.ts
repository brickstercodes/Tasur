/**
 * WHY: Vercel AI SDK fallback Web Search Augmentor Agent implementation.
 *
 * Fetches Tavily search results for each gap directly (no LLM tool-calling),
 * then uses generateObject() to structure the raw results into WebSearchOutput.
 * This avoids passing `tools` to generateObject() which is unsupported in
 * AI SDK v6. The Mastra path follows the same direct-fetch approach.
 * Gracefully returns empty augmentations when TAVILY_API_KEY is absent.
 */
...
async function searchTavily(query: string): Promise<TavilyResult[]> {
  ...
  const response = await fetch(TAVILY_ENDPOINT, {
    ...
    body: JSON.stringify({ api_key: apiKey, query, max_results: RESULTS_PER_GAP, search_depth: 'basic' }),
  });
...
export class ManualWebSearchAgent implements TasurAgent<WebSearchInput, WebSearchOutput> {
  async execute(input: WebSearchInput): Promise<AgentResult<WebSearchOutput>> {
    ...
    // Step 1: Fetch results for each gap directly (no LLM tool-calling needed).
    ...
    // Step 2: Format results as context and use LLM to structure into WebSearchOutput.
